 ...
 */
public class PersonGrideView extends ViewGroup {
    ...
    public PersonGrideView(Context context) {
        super(context);
        initView(context, null);
    }

    public PersonGrideView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context, attrs);
    }

    public PersonGrideView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public PersonGrideView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (size == 1){
            ...
            if (singleImageMode == SINGLE_IMAGE_MODE_RELATIVE_TO_SELF){
                ...
                if (view != null){
                    ...
                    int bitmaoHeight = rect.height();
                    grideWidth = bitmapWidth;
                    grideHeight = bitmaoHeight;
                    if (grideWidth >= totalWidth){
                        grideWidth = totalWidth;
                        grideHeight = grideWidth * bitmaoHeight / bitmapWidth;
                    }
                }
            }
        }else {
    ...
    @Override
    protected void onLayout(boolean boolea, int l, int t, int r, int b) {
        ...
        for (int j = 0; j < size; j++){
            ...
            if (view ==null){
                if (imageCreator == null){
                   imageCreator = (ImageCreator) DefaultImageCreator.getInstance();
                }
                ...
                view.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (onItemClickListener != null){
                            onItemClickListener.onClickItem(position, view);
                        }
                    }
                });
            }
            ...
            l = j % colums * (tmpWidth + space) + getPaddingLeft();
            t = j / colums * (tmpHeight + space) + getPaddingTop();
            r = l + tmpWidth;
            b = t + tmpHeight;
            view.layout(l, t, r, b);
        }
    ...
    private void initView(Context context, AttributeSet attrs){
        ...
        int defaultSpacee = DensityUtil.dp2px(context, DEFAULT_SPACING);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LGNineGrideView);
        space = (int) typedArray.getDimension(R.styleable.LGNineGrideView_space, defaultSpacee);
